...
from database.auth import get_usuario_logado, get_admin
...
def form_nova_movimentacao(
    ...
    return templates.TemplateResponse(
        ...
        "movimentacoes/form.html",
        {
            ...
            "produtos":   produtos,
            "produto_id": produto_id,
            "tipos":      Tipo_movimentacao,  # passa o enum para o template
        }
    )
...
def registrar_movimentacao(
    ...
    if tipo not in (Tipo_movimentacao.ENTRADA, Tipo_movimentacao.SAIDA):
        return templates.TemplateResponse(
            request,
            "movimentacoes/form.html",
            {
    ...
    if tipo == Tipo_movimentacao.SAIDA and quantidade > produto.estoque_atual:
        return templates.TemplateResponse(
            request,
            "movimentacoes/form.html",
            {
...
def historico_produto(
    ...
    total_entradas = sum(
        ...
        if m.tipo == Tipo_movimentacao.ENTRADA
    )
    total_saidas = sum(
        m.quantidade for m in movimentacoes
        if m.tipo == Tipo_movimentacao.SAIDA
    )
